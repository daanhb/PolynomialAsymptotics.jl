% lagpts.m from chebfun [ Driscoll, T. A and Hale, N. and Trefethen, L. N.; Pafnuty Publications; Chebfun Guide; http://www.chebfun.org ] with some additional testing options
function [x, w, v] = lagptsTest(n, int, meth, alf)
...
% Decide to use GW, recurrence, GLR, EXP or RH.
normalize = 1;
if strcmpi(method,'GW')
    ...
    display('Golub-Welsh algo');
    
elseif any(strcmpi(method, {'RH', 'RHW'}))
    % RH, see [3] and [5]
    [x, w] = newton(n, strcmpi(method, 'RHW'), alpha, 1);
    display('std RH newton');
    
elseif any(strcmpi(method, {'RHbes', 'RHWbes'}))
    [x, w] = newton(n, strcmpi(method, 'RHWbes'), alpha, 2);
    normalize = 0;
    display('RHbes');
elseif any(strcmpi(method, {'RHbulk', 'RHWbulk'}))
    [x, w] = newton(n, strcmpi(method, 'RHWbulk'), alpha, 3);
    normalize = 0;
    display('RHbulk');
elseif any(strcmpi(method, {'RHairy', 'RHWairy'}))
    [x, w] = newton(n, strcmpi(method, 'RHWairy'), alpha, 4);
    normalize = 0;
    display('RHairy');
...
elseif strcmpi(method,'fast')
    ...
    v = -v./max(abs(v));
    display('GLR fast algo');

else
    % Explicit expansions, see [4]
    [x,w] = laguerreExp(n, strcmpi(method,'expW'), alpha);
    display('Explicit expansions');
end


if normalize % Do not normalise for bulk for example because large errors on weights near endpoints
...
for k = 1:mn
    ...
    while ( ( abs(step) > eps*400*x(k) ) && ( l < 9) )
        ...
        elseif rh == 2
            pe = asyBessel(n, x(k), alpha, T); %polyAsyRH(n, x(k), alpha, T);
            step = pe/(asyBessel(n-1, x(k), alpha+1, T)*factorx - pe/2);
        elseif rh == 3
            pe = asyBulk(n, x(k), alpha, T);
            step = pe/(asyBulk(n-1, x(k), alpha+1, T)*factorx - pe/2);
        elseif rh == 4
            pe = asyAiry(n, x(k), alpha, T); 
            step = pe/(asyAiry(n-1, x(k), alpha+1, T)*factorx - pe/2);
        elseif rh == 0
    ...
    if ( x(k) < 0 ) || ( x(k) > 4*n + 2*alpha + 2 ) || ( l == 9 ) || ...
            ( ( k ~= 1 ) && ( x(k - 1) >= x(k) ) )
        warning('CHEBFUN:lagpts:converge','Newton method may not have converged.');
    end
    if noUnderflow && (rh == 1)
        w(k) = factorw/polyAsyRH(n-1, x(k), alpha + 1, T)/...
            polyAsyRH(n+1, x(k), alpha, T)/exp( x(k) );
    elseif noUnderflow && (rh == 2)
        w(k) = factorw/asyBessel(n-1, x(k), alpha + 1, T)/...
            asyBessel(n+1, x(k), alpha, T)/exp( x(k) );
    elseif noUnderflow && (rh == 3)
        w(k) = factorw/asyBulk(n-1, x(k), alpha + 1, T)/...
            asyBulk(n+1, x(k), alpha, T)/exp( x(k) );
    elseif noUnderflow && (rh == 4)
        w(k) = factorw/asyAiry(n-1, x(k), alpha + 1, T)/...
            asyAiry(n+1, x(k), alpha, T)/exp( x(k) );
    elseif noUnderflow && (rh == 0)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% %%%%%%%%%%%%%%%%%%%% Routine for explicit expansions %%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     error('Wrong node or weight.') % No error because we may test the expansions in the bulk also near the edges
end
