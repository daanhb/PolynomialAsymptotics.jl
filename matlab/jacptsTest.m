% jacpts.m from chebfun [ Driscoll, T. A and Hale, N. and Trefethen, L. N.; Pafnuty Publications; Chebfun Guide; http://www.chebfun.org ] with some additional testing options, for example:
%n = 161; alpha = -0.7; beta = 1/sqrt(2); [xStd, wStd, vStd] = jacptsTest(n,alpha,beta);
%figure;for T =1:2:7,[xRH,wRH,vRH] =jacptsTest(n,alpha,beta,[-1,1], 'exp', [T,0,n+1]); semilogy(abs(xStd-xRH), 'LineWidth', 2);hold on; end; legend(num2str((1:2:7)'));

function [x, w, v] = jacptsTest(n, a, b, int, meth, param)
...
if ( nargin > 3 )
%     if ( nargin == 5 )
    if ( nargin >= 5 ) % Remove temp extra arg param to change this back to ==
...
if strcmpi(method, 'rh')
    if exist('param', 'var')
        [x, w, v] = newton(n, a, b, param); % RH (Newton on Riemann-Hilbert asy exp)
    else
        [x, w, v] = newton(n, a, b, -1);
    end
elseif strcmpi(method, 'exp') && (nargin == 6)
    [x, w] = jacobiExp(n, a, b, param); % EXP (Explicit asy exp)
    v = 1./sqrt(w'.*(1-x.^2));  % Barycentric weights
...
%REC   Compute nodes and weights using Newton iterations on the recurrrence relation or asymptotic expansions.
...
if rh
    ...
    Uright = Uright(:,:,1:T-1,1:ceil((T-1)/2));
    Uleft = Uleft(:,:,1:T-1,1:ceil((T-1)/2));
    ...
    Ulsh(:,:,2,1) = (4*b^2-1)/256*[Dinf,0;0,Dinf^(-1)]*[-(8*b+8*a-4*a^2+1), ...
        1i*(-8*b-8*a+4*b^2+4*a^2-10) ; 1i*(-8*b-8*a-4*b^2 ...
        -4*a^2+10), -(-8*b-8*a-4*a^2+1)]*[Dinf^(-1),0;0,Dinf]; %B2
    Ursh = Ursh(:,:,1:T-1,1:ceil((T-1)/2));
    Ulsh = Ulsh(:,:,1:T-1,1:ceil((T-1)/2));
    a = a-1; b = b-1;
    
    debug = 1;
    end
    
    [c, d, Dinf] = contour_integrals(a, b, @(x) ones(size(x)), T);
    [Uright,Uleft] = UQ(a,b, Dinf,c, d,T);
    [cs, ds, Dinfs] = contour_integrals(a+1, b+1, @(x) ones(size(x)), T);
    [Ursh,Ulsh] = UQ(a+1, b+1, Dinfs, cs, ds,T);
end
...
% Compute the expansion of the orthonormal polynomial based on some heuristics, np = n or n-1, ...
function p = polyAsyRH(np, x, alpha, beta, Uright, Uleft)
...
for k = 1:length(x)
    psiz = (alpha+beta)/2*acos(x(k)) -alpha*pi/2;
    R = zeros(2,2, T-1);
    for kt = 1:T-1
        for m = 1:round(kt/2)
...
% Compute the expansion of the monic polynomial in the right disk without common factors, might be merged with asyLeft
...
function [x,w] = jacobiExp(n, alpha, beta, param)

if nargin == 4
    il = param(2); ir = param(3);
    T = param(1);
% When il = 0; ir = n+1; then use the bulk for all nodes
else
...
wbulk = t*0;
if T >= 9
    bulk = bulk -1/40320*(219648*alpha^8 - 219648*beta^8 - (9728*alpha^8 + 9728*beta^8 + 896*(138*alpha^2 - 49)*beta^6 ...
        - 43904*alpha^6 + 224*(1160*alpha^4 - 1720*alpha^2 + 389)*beta^4 + 87136*alpha^4 ...
        + 8*(15456*alpha^6 -48160*alpha^4 + 49364*alpha^2 - 9785)*beta^2 - 78280*alpha^2 + 14921)*t.^7 - 10752*(14*alpha^2 - 127)*beta^6 ...
        -40320*(alpha^2 - beta^2)*t.^6 - 1365504*alpha^6 + 21*(2048*alpha^8 + 2048*beta^8 + 128*(146*alpha^2 -123)*beta^6 ...
        - 15744*alpha^6 + 32*(1320*alpha^4 - 1760*alpha^2 + 2023)*beta^4 + 64736*alpha^4 + 8*(2336*alpha^6- 7040*alpha^4 ...
        + 3644*alpha^2 - 11275)*beta^2 - 90200*alpha^2 + 37111)*t.^5 + 75264*(5*alpha^2 - 49)*beta^4 ...
        + 4480*(44*alpha^8 - 44*beta^8 + 8*(3*alpha^2 + 55)*beta^6 - 440*alpha^6 - 24*(7*alpha^2 + 72)*beta^4 ...
        +1728*alpha^4 - (24*alpha^6 - 168*alpha^4 - 2405)*beta^2 - 2405*alpha^2)*t.^4 + 3687936*alpha^4 ...
        + 105*(6656*alpha^8 + 6656*beta^8 - 128*(50*alpha^2 + 443)*beta^6 - 56704*alpha^6 - 32*(296*alpha^4 -696*alpha^2 - 6027)*beta^4 ...
        + 192864*alpha^4 - 8*(800*alpha^6 - 2784*alpha^4 + 3580*alpha^2 + 30285)*beta^2 -242280*alpha^2 + 99933)*t.^3 ...
        + 384*(392*alpha^6 - 980*alpha^4 + 10527)*beta^2 + 2688*(424*alpha^8 - 424*beta^8+ 4*(4*alpha^2 + 783)*beta^6 - 3132*alpha^6 ...
        + 4*(35*alpha^2 - 2407)*beta^4 + 9628*alpha^4 - (16*alpha^6 +140*alpha^4 - 11429)*beta^2 - 11429*alpha^2)*t.^2 ...
        - 4042368*alpha^2 + 35*(23552*alpha^8 + 23552*beta^8 +128*(90*alpha^2 - 1231)*beta^6 - 157568*alpha^6 ...
        + 32*(328*alpha^4 - 1376*alpha^2 + 14095)*beta^4 +451040*alpha^4 + 8*(1440*alpha^6 - 5504*alpha^4 + 9964*alpha^2 - 65439)*beta^2 ...
        - 523512*alpha^2 +206379)*t)*d^8./(t.^2 - 1).^3;
    % Too much memory for intermediate computations in sage to get this higher order term for the weight
end
if T >= 7
    ...
    wbulk = wbulk +1/120*((96*alpha^6 + 96*beta^6 + 80*(8*alpha^2 - 3)*beta^4 - 240*alpha^4 + 2*(320*alpha^4 - 440*alpha^2 +101)*beta^2 ...
        + 202*alpha^2 - 39)*t.^6 - 1440*alpha^6 - 1440*beta^6 - 240*(4*alpha^2 - 23)*beta^4 - 5*(96*alpha^6+ 96*beta^6 ...
        + 16*(20*alpha^2 - 27)*beta^4 - 432*alpha^4 + 2*(160*alpha^4 - 136*alpha^2 + 295)*beta^2 + 590*alpha^2 - 237)*t.^4 ...
        + 5520*alpha^4 - 640*(3*alpha^6 - 3*beta^6 + (alpha^2 + 15)*beta^4 - 15*alpha^4 ...
        -(alpha^4 + 22)*beta^2 + 22*alpha^2)*t.^3 - 30*(32*alpha^4 - 72*alpha^2 + 223)*beta^2 - 15*(288*alpha^6 +288*beta^6 ...
        - 16*(8*alpha^2 + 81)*beta^4 - 1296*alpha^4 - 2*(64*alpha^4 - 88*alpha^2 - 863)*beta^2 +1726*alpha^2 - 717)*t.^2 ...
        - 6690*alpha^2 - 128*(33*alpha^6 - 33*beta^6 - 5*(alpha^2 - 27)*beta^4 - 135*alpha^4 +(5*alpha^4 - 166)*beta^2 + 166*alpha^2)*t ...
        + 2595)*d^6./(t.^6 - 3*t.^4 + 3*t.^2 - 1);
end
if T >= 5
    ...
    wbulk = wbulk + 1/12*((16*alpha^4 + 16*beta^4 + 4*(12*alpha^2 - 5)*beta^2 - 20*alpha^2 + 5)*t.^4 ...
        + 48*alpha^4 + 48*beta^4 + 12*(4*alpha^2 - 7)*beta^2 - 6*(4*(4*alpha^2 + 1)*beta^2 ...
        + 4*alpha^2 - 3)*t.^2 - 84*alpha^2 + 64*(alpha^4 - beta^4 - 2*alpha^2 + 2*beta^2)*t +33)./(t.^4 - 2*t.^2 + 1)*d^4;
end
if T >= 3
    bulk = bulk + 1/2*(2*alpha^2 - 2*beta^2 + (2*alpha^2 + 2*beta^2 - 1)*t)*d^2;
    wbulk = wbulk - (1 -2*alpha^2 -2*beta^2)*d^2;
end
...
w = transpose((1-x).^alpha.*(1+x).^beta.*[wL; pi*d*sqrt(1 -t.^2).*(2+wbulk); flipud(wR)]);

end


% Evaluate the asymptotic expansions of ix nodes and weights in one of the disks: default left disk, else switch alpha and beta and do x = -x
...
d = 1/(2*n+alpha+beta+1);
if T >= 9
    x = x + 2/42525*(6*jbk.^8 - 15*(9*alpha^2 + 7*beta^2 - 4)*jbk.^6 + (6615*alpha^4 + 769*beta^4 + 2*(1620*alpha^2 -589)*beta^2 ...
        - 12150*alpha^2 + 2668)*jbk.^4 + 3*(9450*alpha^6 - 999*beta^6 - 23*(400*alpha^2 - 147)*beta^4 ...
        -40635*alpha^4 - (2835*alpha^4 + 1850*alpha^2 - 294)*beta^2 + 50650*alpha^2 - 10236)*jbk.^2 + ...
        42525*alpha^8 + 2327*beta^8 + 22340*(3*alpha^2 - 1)*beta^6 - 226800*alpha^6 + 168*(1530*alpha^4 - 2415*alpha^2 + 542)*beta^4 ...
        + 517860*alpha^4 + 20*(11340*alpha^6 - 38745*alpha^4 + 42399*alpha^2 - 8488)*beta^2 - 509280*alpha^2 +98717)*d^8;
end
if T >= 7
